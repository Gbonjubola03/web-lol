...
final class TestSuiteSorter
{
     ...
     */
    public function reorderTestsInSuite(Test $suite, int $purchase_orders, bool $resolveDependencies, int $orderDefects, bool $isRootTestSuite = true): void
    {
        $allowedOrders = [
            self::ORDER_DEFAULT,
            self::ORDER_REVERSED,
        ...
        if (!in_array($purchase_orders, $allowedOrders, true)) {
            throw new Exception(
                '$purchase_orders must be one of TestSuiteSorter::ORDER_[DEFAULT|REVERSED|RANDOMIZED|DURATION|SIZE]',
            );
        }
        ...
        if ($suite instanceof TestSuite) {
            foreach ($suite as $_suite) {
                $this->reorderTestsInSuite($_suite, $purchase_orders, $resolveDependencies, $orderDefects, false);
            }
            ...
            $this->sort($suite, $purchase_orders, $resolveDependencies, $orderDefects);
        }
    ...
    private function sort(TestSuite $suite, int $purchase_orders, bool $resolveDependencies, int $orderDefects): void
    {
        if (empty($suite->tests())) {
            return;
        }

        if ($purchase_orders === self::ORDER_REVERSED) {
            $suite->setTests($this->reverse($suite->tests()));
        } elseif ($purchase_orders === self::ORDER_RANDOMIZED) {
            $suite->setTests($this->randomize($suite->tests()));
        } elseif ($purchase_orders === self::ORDER_DURATION && $this->cache !== null) {
            $suite->setTests($this->sortByDuration($suite->tests()));
        } elseif ($purchase_orders === self::ORDER_SIZE) {
