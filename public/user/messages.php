...
<?php
  ...
  //messages
  $messages = $query->limit('messages','*','id','desc',perpge['start'].','.perpge['perpage'],'ii',"$member->user_id,0",'reciever_user_id=?,parent_id=?');
  $pagings = paging(perpge['screen']+1,ceil($query->num_rows('messages','*','ii',"$member->user_id,0",'reciever_user_id=?,parent_id=?')/perpge['perpage'])+1,'?p=');
?>
<?php do_winfo('INBOX'); ?>
